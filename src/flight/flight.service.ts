...
//           isLatest: item.isLatest || 'N/A'
  ...
  // Method to get raw data (for debugging purposes)
  async getRawFlightData(queryDto: FlightQueryDto): Promise<FlightApiResponse> {
    try {
      // Validate that all required parameters are provided
      ...
      const url = this.buildApiUrl(queryDto);
      const response = await firstValueFrom(
        this.httpService.get<FlightApiResponse>(url)
      );
      ...
      this.logger.error('Error fetching raw flight data:', error);
      throw new HttpException(
        error.message || 'Failed to fetch raw data from external API',
        error.status || HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
  ...
  /**
   * Get raw flight version data (for debugging)
   */
  async getRawFlightVersionData(date: string): Promise<FlightVersionApiResponse> {
    try {
      if (!date?.trim()) {
        throw new HttpException(
          'Date parameter is required',
      ...
      const url = this.buildFlightVersionUrl(date.trim());
      const response = await firstValueFrom(
        this.httpService.get<FlightVersionApiResponse>(url, {
          timeout: 15000,
          headers: {
      ...
      this.logger.error('Error fetching raw flight version data:', error);
      throw new HttpException(
        error.message || 'Failed to fetch raw flight version data from external API',
        error.status || HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }

  // Method to test API connectivity with sample data
  async testApiConnection(): Promise<{ status: string; message: string; details?: any }> {
    try {
      // Test with the exact parameters from your original URL
      const testQuery: FlightQueryDto = {
        flightNumber: '123, 456',
        departureLocation: 'DEL, BOM',
        arrivalLocation: 'BLR, CCU',
        departureDate: '2025-07-20'
      };

      this.logger.log('Testing API connection with all required parameters...');
      const result = await this.getFlightVersions(testQuery);
      return {
        status: 'success',
  ...
  /**
   * Test flight version API connectivity
   */
  async testFlightVersionApi(): Promise<{ status: string; message: string; details?: any }> {
    try {
      const testDate = '2025-07-15';
      this.logger.log(`Testing flight version API with date: ${testDate}`);

      const result = await this.getFlightVersionsByDate({ date: testDate });

      return {
        status: 'success',
        message: 'Flight version API connection successful',
        details: {
          recordCount: result.length,
          sampleData: result.slice(0, 2) // Show first 2 records
        }
      };
    } catch (error) {
      this.logger.error('Flight version API connection test failed:', error);
      return {
        status: 'error',
        message: `Flight version API connection failed: ${error.message}`,
        details: error.response ? {
          status: error.response.status,
          statusText: error.response.statusText,
          data: error.response.data
  ...
  // Method to manually test with exact URL construction
  async testWithManualUrl(): Promise<any> {
    try {
      // Build URL exactly like the working example
      const manualUrl = `${this.baseUrl}/versions?flightNumber=123%2C%20456&departureLocation=DEL%2C%20BOM&departureDate=2025-07-20&arrivalLocation=BLR%2C%20CCU`;
      this.logger.log(`Testing with manual URL: ${manualUrl}`);

      const response = await firstValueFrom(
        this.httpService.get(manualUrl, {
          timeout: 15000,
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
      ...
      this.logger.error('Manual URL test failed:', error);
      return {
        status: 'error',
        url: `${this.baseUrl}/versions?flightNumber=123%2C%20456&departureLocation=DEL%2C%20BOM&departureDate=2025-07-20&arrivalLocation=BLR%2C%20CCU`,
